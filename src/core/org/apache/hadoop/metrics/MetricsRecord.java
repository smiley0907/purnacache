 ...
 * program reporting statistics relating to the disks on a computer might use a
 * record name "diskStats".
 * <p/>
 * 
 * A record has zero or more <i>tags</i>. A tag has a name and a value. To
 * continue the example, the "diskStats" record might use a tag named "diskName"
 * to identify a particular disk. Sometimes it is useful to have more than one
 * tag, so there might also be a "diskType" with value "ide" or "scsi" or
 * whatever.
 * <p/>
 * 
 * A record also has zero or more <i>metrics</i>. These are the named values
 * that are to be reported to the metrics system. In the "diskStats" example,
 * possible metric names would be "diskPercentFull", "diskPercentBusy",
 * "kbReadPerSecond", etc.
 * <p/>
 * 
 ...
 * client library. Metric data is not immediately sent to the metrics system
 * each time that <code>update()</code> is called. An internal table is
 * maintained, identified by the record name. This table has columns
 * corresponding to the tag and the metric names, and rows corresponding to each
 * unique set of tag values. An update either modifies an existing row in the
 * table, or adds a new row with a set of tag values that are different from all
 * the other rows. Note that if there are no tags, then there can be at most one
 * row in the table.
 * <p/>
 * 
 ...
 * <code>remove()</code> method can be used to remove the row and thus stop the
 * data from being sent.
 * <p/>
 * 
 * Note that the <code>update()</code> method is atomic. This means that it is
 * safe for different threads to be updating the same metric. More precisely, it
 * is OK for different threads to call <code>update()</code> on MetricsRecord
 * instances with the same set of tag names and tag values. Different threads
 * should <b>not</b> use the same MetricsRecord instance at the same time.
 */
public interface MetricsRecord {
	 ...
	 *             if the tagName conflicts with the configuration
	 */
	 ...
	 * @throws MetricsException
	 *             if the metricName or the type of the metricValue conflicts
	...
	/**
	 * Updates the table of buffered data which is to be sent periodically. If
	 * the tag values match an existing row, that row is updated; otherwise, a
	 * new row is added.
	 */
	 ...
	 * the tags that have been set on this record. For example, if there are no
	 * tags on this record, all rows for this record name would be removed. Or,
	 * if there is a single tag on this record, then just rows containing a tag
	 * with the same name and value would be removed.
	 */
