 ...
 * extended in order to integrate the Metrics API with a specific metrics client
 * library.
 * <p/>
 * 
 * This class implements the internal table of metric data, and the timer on
 * which data is to be sent to the metrics system. Subclasses must override the
 * abstract <code>emitRecord</code> method in order to transmit the data.
 * <p/>
 */
public abstract class AbstractMetricsContext implements MetricsContext {
	 ...
	 * <code>recordName</code>. Throws an exception if the metrics
	 * implementation is configured with a fixed set of record names and
	 * <code>recordName</code> is not in that set.
	 * 
	 * @param recordName
	...
	private synchronized void startTimer() {
		if (timer == null) {
			timer = new Timer(
					"Timer thread for monitoring " + getContextName(), true);
	 ...
	 * nothing. Subclasses may override it in order to perform some kind of
	 * flush.
	 */
	...
	/**
	 * Called by MetricsRecordImpl.remove(). Removes all matching rows in the
	 * internal table of metric data. A row matches if it has the same tag names
	 * and values as record, but it may also have additional tags.
	 */
