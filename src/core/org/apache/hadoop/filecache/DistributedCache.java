 ...
 * <code>DistributedCache</code> assumes that the files specified via hdfs://
 * urls are already present on the {@link FileSystem} at the path specified by
 * the url.
 * </p>
 * 
 * <p>
 * The framework will copy the necessary files on to the slave node before any
 ...
 * software distribution mechanism. Files have execution permissions. Optionally
 * users can also direct it to symlink the distributed cache file(s) into the
...
public class DistributedCache {
	...
	/**
	 * Get the locally cached file or archive; it could either be previously
	 * cached (and valid) or copy it from the {@link FileSystem} now.
	 * 
	 * @param cache
	 *            the cache to be localized, this should be specified as new
	 *            URI(hdfs://hostname:port/absolute_path_to_file#LINKNAME). If
	 *            no schema or hostname:port is provided the file is assumed to
	 *            be in the filesystem being used in the Configuration
	 * @param conf
	 *            The Confguration file which contains the filesystem
	 * @param baseDir
	 *            The base cache Dir where you wnat to localize the
	 *            files/archives
	 * @param fileStatus
	 *            The file status on the dfs.
	 * @param isArchive
	 *            if the cache is an archive or a file. In case it is an archive
	 *            with a .zip or .jar or .tar or .tgz or .tar.gz extension it
	 *            will be unzipped/unjarred/untarred automatically and the
	 *            directory where the archive is unzipped/unjarred/untarred is
	 *            returned as the Path. In case of a file, the path to the file
	 *            is returned
	 * @param confFileStamp
	 *            this is the hdfs file modification timestamp to verify that
	 *            the file to be cached hasn't changed since the job started
	...
	public static Path getLocalCache(URI cache, Configuration conf,
			Path baseDir, FileStatus fileStatus, boolean isArchive,
			long confFileStamp, Path currentWorkDir) throws IOException {
	 ...
	 *            this is the directory where you would want to create symlinks
	 *            for the locally cached files/archives
	 * @param honorSymLinkConf
	 *            if this is false, then the symlinks are not created even if
	 *            conf says so (this is required for an optimization in task
	 *            launches
	 * @return the path to directory where the archives are unjarred in case of
	 *         archives, the path to the file where the file is copied locally
	 * @throws IOException
	...
	public static Path getLocalCache(URI cache, Configuration conf,
			Path baseDir, FileStatus fileStatus, boolean isArchive,
			long confFileStamp, Path currentWorkDir, boolean honorSymLinkConf)
			throws IOException {
		...
		synchronized (cachedArchives) {
			...
			if (lcacheStatus == null) {
				// was never localized
				lcacheStatus = new CacheStatus(baseDir, new Path(baseDir,
						new Path(cacheId)));
			...
			synchronized (lcacheStatus) {
				localizedPath = localizeCache(conf, cache, confFileStamp,
						lcacheStatus, fileStatus, isArchive, currentWorkDir,
						honorSymLinkConf);
	 ...
	 */
	public static Path getLocalCache(URI cache, Configuration conf,
			Path baseDir, boolean isArchive, long confFileStamp,
			Path currentWorkDir) throws IOException {
		return getLocalCache(cache, conf, baseDir, null, isArchive,
				confFileStamp, currentWorkDir);
	}

	/**
	 * This is the opposite of getlocalcache. When you are done with using the
	 * cache, you need to release the cache
	 * 
	 * @param cache
	 *            The cache URI to be released
	 * @param conf
	 *            configuration which contains the filesystem the cache is
	 *            contained in.
	 * @throws IOException
	 */
				...
				synchronized (lcacheStatus) {
					if (lcacheStatus.refcount == 0) {
						...
						FileSystem.getLocal(conf).delete(
								lcacheStatus.localLoadPath, true);
						synchronized (baseDirSize) {
							Long dirSize = baseDirSize
									.get(lcacheStatus.baseDir);
							if (dirSize != null) {
	...
	// the method which actually copies the caches locally and unjars/unzips
	// them
	// and does chmod for the files
	private static Path localizeCache(Configuration conf, URI cache,
			long confFileStamp, CacheStatus cacheStatus, FileStatus fileStatus,
			boolean isArchive, Path currentWorkDir, boolean honorSymLinkConf)
			throws IOException {
		...
		if (ifExistsAndFresh(conf, fs, cache, confFileStamp, cacheStatus,
			...
			if (isArchive) {
				...
				if (doSymlink) {
					if (!flink.exists())
						FileUtil.symLink(cacheFilePath(
								cacheStatus.localLoadPath).toString(), link);
				}
			...
			long cacheSize = FileUtil.getDU(new File(parchive.getParent()
					.toString()));
		...
		if (isArchive) {
			if (doSymlink) {
				if (!flink.exists())
					FileUtil
							.symLink(cacheStatus.localLoadPath.toString(), link);
			}
			return cacheStatus.localLoadPath;
		} else {
			if (doSymlink) {
	...
	private static boolean isTarFile(String filename) {
		return (filename.endsWith(".tgz") || filename.endsWith(".tar.gz") || filename
				.endsWith(".tar"));
	}

	// Checks if the cache has already been localized and is fresh
	private static boolean ifExistsAndFresh(Configuration conf, FileSystem fs,
			URI cache, long confFileStamp, CacheStatus lcacheStatus,
	...
	public static void createAllSymlink(Configuration conf, File jobCacheDir,
		...
		if (createSymlink) {
			...
			for (int i = 0; i < list.length; i++) {
				FileUtil.symLink(list[i].getAbsolutePath(), new File(workDir,
						list[i].getName()).toString());
			}
		}
	}
     ...
     *            Configuration which will be changed
     */
    public static void setCacheFilesNSet(URI[] files, Cache cache) {
        for (URI uri : files) {
            cache.put(uri.hashCode(), uri);
        }
    }
    ...
    public static Object getCacheFilesNSet(Cache cache, int key) throws IOException {
	...
	public static URI[] getCacheArchives(Configuration conf) throws IOException {
		return StringUtils
				.stringToURI(conf.getStrings("mapred.cache.archives"));
	}
	...
	public static String getCacheFilesByIndex(Configuration conf, int index) throws IOException {
	    String [] strs = conf.getStrings("mapred.cache.files");
	    return strs[index];
    }
	...
	public static Path[] getLocalCacheFiles(Configuration conf)
			throws IOException {
		return StringUtils.stringToPath(conf
				.getStrings("mapred.cache.localFiles"));
	}
	 ...
	 * @param timestamps
	 *            comma separated list of timestamps of archives. The order
	 *            should be the same as the order in which the archives are
	 *            added.
	 */
	 ...
	 *            Configuration which stores the timestamp's
	 * @param timestamps
	 *            comma separated list of timestamps of files. The order should
	 *            be the same as the order in which the files are added.
	 */
	...
	/**
	 * This method checks to see if symlinks are to be create for the localized
	 * cache files in the current working directory
	 * 
	 * @param conf
	 *            the jobconf
	...
	/**
	 * This method checks if there is a conflict in the fragment names of the
	 * uris. Also makes sure that each uri has a fragment. It is only to be
	 * called if you want to create symlinks for the various archives and files.
	 * 
	 * @param uriFiles
