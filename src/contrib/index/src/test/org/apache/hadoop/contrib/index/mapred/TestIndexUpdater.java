...
public class TestIndexUpdater extends TestCase {
	...
	private Path localInputPath = new Path(System.getProperty("build.test")
			+ "/sample/data.txt");
	...
	protected void setUp() throws Exception {
		super.setUp();
		try {
			dfsCluster = new MiniDFSCluster(conf, numDataNodes, true,
					(String[]) null);
			...
			mrCluster = new MiniMRCluster(numTaskTrackers, fs.getUri()
					.toString(), 1);
	...
	public void testIndexUpdater() throws IOException {
		...
		// max field length, compound file and number of segments will be
		// checked
		// later
		...
		for (int i = 0; i < numRuns; i++) {
			...
			for (int j = 0; j < shards.length; j++) {
				shards[j] = new Shard(versionNumber, new Path(indexPath,
	...
	private void run(int numRuns, Shard[] shards) throws IOException {
		...
		for (int i = 0; i < shards.length; i++) {
			Directory dir = new FileSystemDirectory(fs, new Path(shards[i]
					.getDirectory()), false, conf);
		...
		Hits hits = searcher
				.search(new TermQuery(new Term("content", "apache")));
		...
		for (int i = 0; i < shards.length; i++) {
			Directory dir = new FileSystemDirectory(fs, new Path(shards[i]
					.getDirectory()), false, conf);
			IndexWriter writer = new IndexWriter(dir, false, null,
					new KeepOnlyLastCommitDeletionPolicy());
			writer.close();
		}

		// verify the number of segments, must be done after an writer with
		// KeepOnlyLastCommitDeletionPolicy so that earlier checkpoints are
		// removed
		for (int i = 0; i < shards.length; i++) {
			...
			FileStatus[] cfsFiles = fs.listStatus(new Path(shards[i]
					.getDirectory()), cfsFilter);
