...
/**
 * A utility class which writes an index in a ram dir into a DataOutput and read
 * from a DataInput an index into a ram dir.
 */
			...
			if (length > 0) {
				...
				try {
					...
					while (position < length) {
						int len = position + BUFFER_SIZE <= length ? BUFFER_SIZE
								: (int) (length - position);
