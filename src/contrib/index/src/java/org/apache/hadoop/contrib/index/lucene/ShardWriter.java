 ...
 * successfully creating the new version in the temp dir, the shard writer moves
 * the new files to the perm dir and deletes the temp dir in close().
 */
public class ShardWriter {
	...
	public ShardWriter(FileSystem fs, Shard shard, String tempDir,
		...
		dir = new MixedDirectory(fs, perm, localFs, fs.startLocalOutput(perm,
				temp), iconf.getConfiguration());

		// analyzer is null because we only use addIndexes, not addDocument
		writer = new IndexWriter(dir, false, null,
	 ...
	 * closing if necessary, and copy the files created in the temp directory to
	 * the permanent directory after closing.
	 * 
	 * @throws IOException
	 */
		...
		try {
			try {
				if (maxNumSegments > 0) {
					...
					LOG.info("Optimized the shard into at most "
							+ maxNumSegments + " segments");
				}
			} finally {
				writer.close();
	...
	private void restoreGeneration(FileSystem fs, Path perm, long startGen)
		...
		for (int i = 0; i < fileStatus.length; i++) {
			...
			if (startGen < LuceneUtil.generationFromSegmentsFileName(path
					.getName())) {
				fs.delete(path);
			}
		}
	...
	private void moveFromTempToPerm() throws IOException {
		try {
			...
			// move the files created in temp dir except segments_N and
			// segments.gen
			for (int i = 0; i < fileStatus.length; i++) {
				Path path = fileStatus[i].getPath();
			...
			if (segmentsGenPath != null) {
				fs.completeLocalOutput(
						new Path(perm, segmentsGenPath.getName()),
						segmentsGenPath);
			}
		} finally {
