 ...
 * a Lucene instance. This class contains the main() method which uses a
 ...
 * updating the shards: - Input formatter. This specifies how to format the
 * input documents. - Analysis. This defines the analyzer to use on the input.
 * The analyzer determines whether a document is being inserted, updated, or
 * deleted. For inserts or updates, the analyzer also converts each input
 * document into a Lucene document. - Input paths. This provides the location(s)
 * of updated documents, e.g., HDFS files or directories, or HBase tables. -
 * Shard paths, or index path with the number of shards. Either specify the path
 * for each shard, or specify an index path and the shards are the
 * sub-directories of the index directory. - Output path. When the update to a
 * shard is done, a message is put here. - Number of map tasks.
 * 
 * All of the information can be specified in a configuration file. All but the
 * first two can also be specified as command line options. Check out
 * conf/index-config.xml.template for other configurable parameters.
 * 
 * Note: Because of the parallel nature of Map/Reduce, the behaviour of multiple
 * inserts, deletes or updates to the same document is undefined.
 */
public class UpdateIndex {
	...
	private static Shard[] createShards(String indexPath, int numShards,
		...
		if (fs.exists(path)) {
			...
			for (int i = 0; i < count; i++) {
				shards[i] = new Shard(versionNumber, parent + shardNames[i],
						generation);
			}
			...
			for (int i = 0; i < shards.length; i++) {
				shards[i] = new Shard(versionNumber, parent
						+ NUMBER_FORMAT.format(i), generation);
			}
			return shards;
		}
	}
	...
	public static void main(String[] argv) {
		...
		try {
			...
			long startTime = now();
			try {
				IIndexUpdater updater = (IIndexUpdater) ReflectionUtils
						.newInstance(iconf.getIndexUpdaterClass(), conf);
