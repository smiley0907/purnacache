 ...
 * (FileSystemDirectory). Index files created by newer versions are written to a
 * writable local FS dir (Lucene's FSDirectory). We should use the general
 ...
 * 
 * Note: We may delete files from the read-only FileSystem dir because there can
 * be some segment files from an uncommitted checkpoint. For the same reason, we
 * may create files in the writable dir which already exist in the read-only dir
 * and logically they overwrite the ones in the read-only dir.
 */
