...
/**
 * An implementation of ChecksumFileSystem over DistributedFileSystem. Note that
 * as of now (May 07), DistributedFileSystem natively checksums all of its data.
 * Using this class is not be necessary in most cases. Currently provided mainly
...
public class ChecksumDistributedFileSystem extends ChecksumFileSystem {
	...
	/**
	 * Any extra interface that DistributeFileSystem provides can be accessed
	 * with this.
	 */
	...
	/**
	 * We need to find the blocks that didn't match. Likely only one is corrupt
	 * but we will report both to the namenode. In the future, we can consider
	 * figuring out exactly which block is corrupt.
	 */
