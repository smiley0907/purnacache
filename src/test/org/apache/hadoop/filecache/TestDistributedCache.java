...
public class TestDistributedCache extends TestCase {
	...
	private static String TEST_CACHE_BASE_DIR = new Path(System.getProperty(
			"test.build.data", "cachebasedir\\")).toString().replace(' ',
			'+');
	private static String TEST_ROOT_DIR = System.getProperty("test.build.data",
			"C:\\Users\\yelisetr\\Downloads\\Purna\\distributedcache\\");
	...
	public void testDeleteCache() throws Exception {
		DistributedCache.getLocalCache(firstCacheFile.toUri(), conf, new Path(
				TEST_CACHE_BASE_DIR), false, System.currentTimeMillis(),
				new Path(TEST_ROOT_DIR));
		...
		DistributedCache.getLocalCache(secondCacheFile.toUri(), conf, new Path(
				TEST_CACHE_BASE_DIR), false, System.currentTimeMillis(),
				new Path(TEST_ROOT_DIR));
		FileStatus[] dirStatuses = localfs.listStatus(new Path(
				TEST_CACHE_BASE_DIR));
		assertTrue(
